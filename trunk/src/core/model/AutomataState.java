...
public abstract class AutomataState implements Serializable {
	...
	public static AutomataState createState(Class<? extends AutomataState> targetClass, AutomataState currentState, Class<? extends AutomataState> superclass) {
		...
		try {
			...
			if (as != null) {
				...
				nextState = targetClass.newInstance();
	...
	private static AutomataState copyAtributeValues(Class<? extends AutomataState> stateClass, AutomataState oldState, AutomataState newState) {
		...
		for (Field f : stateClass.getDeclaredFields()) {
			...
			if (getterMethod != null) {
				try {
					...
					Logger.println(String.format("Error copying AutomataState [class:%s][attribute:%s][method:%s] %s", stateClass.getName(), f.getName(), getterMethod.getName(), e.getMessage()));
					e.printStackTrace();
				}
			}
		}
